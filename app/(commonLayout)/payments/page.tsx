...
function PaymentCard({
  ...
  return (
    <Card
      className={`w-full md:w-[350px] ${height} bg-white/5 backdrop-blur-md border ${
        highlight ? 'border-yellow-400' : 'border-teal-600'
      ...
      <CardContent className="flex flex-col gap-4 pt-0 flex-grow">
        ...
        <div className="mt-auto">
          ...
          <Button
            ...
            className={`w-full text-white text-base py-2 ${
...
export default function PaymentsPage() {
  ...
  const { data: studentData, isLoading: isStudentLoading } = useGetStudentByUserIdQuery(user?.studentId);
  const [paymentInitiate] = usePaymentInitiateMutation();

  const [submittingPlan, setSubmittingPlan] = useState<string | null>(null); // plan title to track submitting

  const handlePaymentSubmit = catchAsync(async (plan: Plan) => {
    if (!plan || !studentData?.data?.id) return;

    setSubmittingPlan(plan.title); // set currently submitting plan
    ...
    try {
      ...
      if (res?.success && res?.data?.paymentInitiateResult?.payment_url) {
        ...
        window.location.href = res.data.paymentInitiateResult.payment_url;
      } else {
      ...
      <section className="min-h-screen py-16 px-4 md:px-10 bg-gradient-to-br from-[#0f172a] to-[#164e63]">
        ...
        <div className="flex flex-wrap justify-center gap-6">
          ...
          <PaymentCard
            ...
            onClickPay={() => handlePaymentSubmit({ title: 'Admission Fee', price: 5000 })}
            isSubmitting={submittingPlan === 'Admission Fee'}
          />
          <PaymentCard
            title="Library Fee"
